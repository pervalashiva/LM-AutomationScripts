...
describe('LoadMiles application',function() 
{
    it("Login with valid UserId Valid Password", async()=>{
       ...
       it("Verify Driver invoice",async()=>{

        //Open DriverPay Invoice
        await  $("//a[normalize-space()='Finance']");
        await   $("//a[normalize-space()='Finance']").click();

        await  $("//app-sidebar-nav-link-content[normalize-space()='Driver Pay']");
        await   $("//app-sidebar-nav-link-content[normalize-space()='Driver Pay']").click();

        // await $("//input[@placeholder='Search Driver']").setValue('shiva');
        // await  browser.pause(1000);
        // await $("//span[normalize-space()='shiva']").click();
        // await $("//button[normalize-space()='Search']").click();
                
        // var drivername = await $("//td[normalize-space()='shiva']");
        // await drivername.waitUntil(
        //     async function () {
        //     return (await this.getText() === 'shiva')
        // }, {
        //     timeout: 10000,
        //     timeoutMsg: 'expected drivername is not same'
        // });
        ...
        var loadCost = await $("tr[class='ng-star-inserted'] td:nth-child(1)");
    ...
    xit("Bulk driverpay invoice ",async()=>{

     await $("//input[@placeholder='Search customer']").setValue('shiva');
     await browser.pause(1000);
...
xit("Verify create invoice Buttons load status",async()=>{
     
    await $("//input[@id='loadNumber']").setValue('1003');
    await $("//button[normalize-space()='Search']").click();
    await browser.pause(2000);
 ...
 it("Increase The load price",async()=>{
     ...
     await  $("//a[@ng-reflect-router-link='/dashboard/driver-pay']");
     await   $("//a[@ng-reflect-router-link='/dashboard/driver-pay']").click();
     await browser.pause(1000);

     await $("//input[@id='loadNumber']").setValue('1014');
     await $("//button[normalize-space()='Search']").click();

     var loadPrice = await $("//td[normalize-space()='1,000']");
     await loadPrice.waitUntil(
         async function () {
...
it("Increase The load cost",async()=>{
