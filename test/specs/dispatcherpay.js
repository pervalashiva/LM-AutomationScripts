...
describe('LoadMiles application',function() 
{
    it("Login with valid UserId Valid Password", async()=>{
       ...
       it("Verify Dispatcher Pay",async()=>{

        //Open Dispatcher Invoice
        await  $("//a[normalize-space()='Finance']");
        await   $("//a[normalize-space()='Finance']").click();

        await  $("//app-sidebar-nav-link-content[normalize-space()='Dispatcher Pay']");
        await   $("//app-sidebar-nav-link-content[normalize-space()='Dispatcher Pay']").click();

        await $("//input[@placeholder='Search Dispatcher']").setValue('Rajesh');
        await  browser.pause(1000);
        await $("//span[normalize-space()='Rajesh']").click();
        await $("//button[normalize-space()='Search']").click();
                
        var dispatcherName = await $("//td[normalize-space()='Rajesh']");
        await dispatcherName.waitUntil(
            async function () {
            return (await this.getText() === 'Rajesh')
        }, {
            timeout: 10000,
            timeoutMsg: 'expected dispatcherName is not same'
        });
        ...
        var loadNumber = await $("//td[normalize-space()='1019']");
        await loadNumber.waitUntil(
            async function () {
            return (await this.getText() === '1019')
        }, {
            timeout: 1000,
    ...
    xit("Bulk dispatcherPay invoice ",async()=>{
...
xit("Verify dispatcher pay Buttons load status",async()=>{
 ...
 it("Decrease The load cost",async()=>{
    ...
    var loadCost = await $("(//td[contains(text(),'100')])[1]");
    await loadCost.waitUntil(
        async function () {
        return (await this.getText() === '100')
    }, {
